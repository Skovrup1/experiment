...
compare_nodes :: proc(p1: ^parser.Parser, n1: parser.NodeIndex, p2: ^parser.Parser, n2: parser.NodeIndex) -> bool {
	...
	if node1.kind != node2.kind {
		fmt.printf("Node kind mismatch: %v vs %v\n", node1.kind, node2.kind)
		return false
	}
	
	#partial switch node1.kind {
	case .Identifier, .Integer, .Primitive:
	...
	case .Procedure:
		...
		if len(d1.parameters) != len(d2.parameters) {
			fmt.printf("Parameter count mismatch: %d vs %d\n", len(d1.parameters), len(d2.parameters))
			return false
		}
	...
	case .Call:
		...
		if len(d1.arguments) != len(d2.arguments) {
			fmt.printf("Call argument count mismatch: %d vs %d\n", len(d1.arguments), len(d2.arguments))
			return false
		}
